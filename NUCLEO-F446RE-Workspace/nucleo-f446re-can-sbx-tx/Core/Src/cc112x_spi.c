...
cc_rx_spi_write_fifo(uint8_t *data, uint8_t *spi_rx_data, size_t len)
{
  ...
  HAL_GPIO_WritePin (GPIOB, GPIO_PIN_12, GPIO_PIN_RESET);
  delay_us(4);
  ret = HAL_SPI_TransmitReceive (&hspi2, data, spi_rx_data, len, 5000);
  HAL_GPIO_WritePin (GPIOB, GPIO_PIN_12, GPIO_PIN_SET);
  return ret;
}
...
cc_rx_spi_read_fifo(uint8_t *out, size_t len)
{
  ...
  ret = HAL_SPI_TransmitReceive (&hspi2, rx_spi_tx_buf, rx_tmp_buf, len + 1, 5000);
  delay_us(4);
...
cc_rx_cmd (uint8_t CMDStrobe)
{
  ...
  HAL_GPIO_WritePin (GPIOB, GPIO_PIN_12, GPIO_PIN_RESET);
  delay_us(4);
  HAL_SPI_TransmitReceive (&hspi2, (uint8_t*) aTxBuffer, (uint8_t *) aRxBuffer, 1, 5000);
  HAL_GPIO_WritePin (GPIOB, GPIO_PIN_12, GPIO_PIN_SET);
  delay_us(4);
  return aRxBuffer[0];
}
